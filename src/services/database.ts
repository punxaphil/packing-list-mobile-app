...
const createAsyncStoragePersistence = (storage: typeof AsyncStorage): PersistenceConstructor => {
...
try {
  ...
  if (message.toLowerCase().includes("already") && message.toLowerCase().includes("auth")) {
...
export const writeDb = {
  ...
  async updatePackItemsBatched(items: PackItem[]) {
    const batch = writeBatch(firestore);
    for (const item of items) {
      batch.update(doc(firestore, USERS_KEY, getUserId(), PACK_ITEMS_KEY, item.id), item as DocumentData);
    }
    await batch.commit();
  },
  addPackItemBatch(
