...
export async function moveListToSpace(sourceSpaceId: string, targetSpaceId: string, listId: string): Promise<void> {
  ...
  const targetCategoryByName = buildEntityNameMap(
    targetCategoriesSnap.docs.map((entry) => ({ id: entry.id, ...entry.data() }))
  );
  ...
  const sourceImages = sourceImagesSnap.docs.map((entry) => ({ id: entry.id, ...entry.data() })) as {
    id: string;
    type: string;
    typeId: string;
    url: string;
  ...
  for (const image of sourceImages) {
    ...
    batch.set(targetImageRef, { type: image.type, typeId: mappedTypeId, url: image.url });
...
export async function ensureUserMemberId(spaceId: string, userId: string, normalizedEmail: string) {
  ...
  if (!hasEmail) return;
  await updateDoc(doc(firestore, SPACES, spaceId), { members: arrayUnion(userId) });
}
