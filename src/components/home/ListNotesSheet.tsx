...
const COPY = {
  title: "List info",
  dueDate: "Due date",
  dueDatePlaceholder: "Set date and time",
  clearDueDate: "Clear",
    ...
    <View style={styles.content}>
      {Platform.OS === "ios" && canEditPackingListReminder ? (
        <View style={styles.fieldGroup}>
          <Text style={styles.fieldLabel}>{COPY.dueDate}</Text>
          <Pressable style={styles.fieldButton} onPress={() => void state.pickDueAt()}>
            <Text style={state.dueAt ? styles.fieldValue : styles.fieldPlaceholder}>
              {state.dueAt ? formatPackingListDueAt(state.dueAt) : COPY.dueDatePlaceholder}
            </Text>
          </Pressable>
          {state.dueAt ? (
            <Pressable hitSlop={8} onPress={state.clearDueAt}>
              <Text style={styles.clearButton}>{COPY.clearDueDate}</Text>
            </Pressable>
          ) : null}
        </View>
      ) : null}
