...
const MemberList = ({
  ...
  return (
    <ScrollView style={[styles.list, { maxHeight: listMaxHeight }]}>
      {members.length === 0 && (
        <Text style={styles.empty}>{COPY.noMembers}</Text>
      )}
      {members.map((member, index) => (
        <MemberRow
          ...
          member={member}
          imageUrl={
            memberImages.find((image) => image.typeId === member.id)?.url
          }
...
const useListMaxHeight = (memberCount: number, iosSheet: boolean) => {
  ...
  const screenCap = Math.floor(
    height * (iosSheet ? IOS_SHEET_LIST_RATIO : DIALOG_LIST_RATIO),
  );
