...
const useDropHandler = (lists: PackingListSummary[], setOrderedIds: OrderedIdsState[1]): DropHandler =>
  useCallback((snapshot, layouts) => {
    ...
    setOrderedIds((current) => {
      const next = reorderIds(current, snapshot, layouts);
...
const reorderIds = (current: string[], snapshot: DragSnapshot, layouts: LayoutMap) => {
  if (!snapshot) return current;
  const from = current.indexOf(snapshot.id);
  if (from < 0) return current;
  const insertIndex = getInsertIndex(current, snapshot, layouts);
  if (insertIndex < 0) return current;
  const next = [...current];
  const [item] = next.splice(from, 1);
  next.splice(clampIndex(insertIndex, from, next.length), 0, item);
  return arraysEqual(next, current) ? current : next;
};

const getInsertIndex = (ids: string[], snapshot: DragSnapshot, layouts: LayoutMap) => {
  if (!snapshot) return -1;
  const draggedLayout = layouts[snapshot.id];
  if (!draggedLayout) return -1;
  const ghostCenter = draggedLayout.y + snapshot.offsetY + draggedLayout.height / 2;
  for (let index = 0; index < ids.length; index += 1) {
    if (ids[index] === snapshot.id) continue;
    const layout = layouts[ids[index]];
    if (!layout) continue;
    const layoutCenter = layout.y + layout.height / 2;
    if (ghostCenter < layoutCenter) return index;
  }
  return ids.length;
};

const clampIndex = (target: number, from: number, length: number) => {
  if (target > length) return length;
  if (target > from) return target - 1;
