...
const sortItems = (items: PackItem[]) =>
  [...items].sort((first, second) => second[ORDER_FIELD] - first[ORDER_FIELD]);
...
const handleSnapshot =
  (sourceKey: string, setState: (value: HookState) => void) =>
...
const subscribeToItems = (
  ...
  onSnapshot(
    buildQuery(spaceId, packingListId),
    handleSnapshot(sourceKey, setState),
    handleError(sourceKey, setState),
  );
...
const manageSubscription = (
  ...
  return (
    subscribeToItems(spaceId, packingListId, sourceKey, setState) ??
    NOOP_UNSUBSCRIBE
  );
};
...
export function usePackingItems(
  ...
  const [state, setState] = useState<HookState>(() =>
    createState(sourceKey, Boolean(sourceKey)),
  );
  ...
  useEffect(
    () => manageSubscription(spaceId, packingListId, setState),
