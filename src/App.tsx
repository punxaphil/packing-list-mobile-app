...
function buildView(params: ViewParams) {
  ...
  return <HomeScreen email={params.email} lists={params.lists} loading={params.loading} hasLists={params.hasLists} userId={params.userId} onSignOut={signOut} />;
}

export default function App() {
  const { userId, email, loggingIn } = useCurrentUser();
  const { packingLists, loading, hasLists } = usePackingLists(userId);
  const view = buildView({ loggingIn, userId, email, lists: packingLists, loading, hasLists });
